...
export class DpxCommsChannel implements ComponentFramework.ReactControl<IInputs, IOutputs> {
    ...
    private props: IHelloWorldProps = { name: 'Hello, World!' };
    ...
    private processNewMessage(voteItemId: number, voteItemCount: number): void {
      console.info(`CommsMessage: ${voteItemId} / ${voteItemCount}`);
      this._output.VoteItemId = voteItemId;
      this._output.VoteItemCount = voteItemCount;
      this.props.name = `Vote Item Id: ${voteItemId} / Vote Item Count: ${voteItemCount}`;
    ...
    {
      this.openConnection();
      return React.createElement(HelloWorld, this.props);
    }
